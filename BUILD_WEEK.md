...
| 2026-07-14 | Explicit Code, Work, and Local modes | `33a5dac` through final integration commit | Current Codex desktop task; `/feedback` ID pending | Eight planned GPT-5.6 Luna roles implemented and reviewed route architecture, Pi Code, Hermes Work, readiness, mode UI, compatibility, and integration boundaries in isolated worktrees. The final Luna xhigh audit found five actionable runtime/authentication issues; the parent repaired all five and an additional refresh/validation completion race. | User-selected modes only; Pi and Hermes stay hidden as implementation runtimes; no forks, cross-mode fallback, ambient credentials, provider-session copying, Grok subagents, or Sol subagents. The Keychain-to-auth-file bridge is exposed only inside a selected persisted direct OpenCode compatibility chat, never to new Pi/Hermes routes. | 843 deterministic fallback checks passed; 38 native test files / 422 declarations were inventoried but not executed; packaged arm64 app verification passed. Real-app QA passed Code, Work, Local, mode locking, slash commands, readiness actions, Connections, runtime ownership disclosure, inspector, icon rail, and deterministic first-frame model rendering. |
| 2026-07-15 | Typed multimodal input and truthful image capability plumbing | This dated commit | Codex task delegated from `019f6235-6ecb-76f0-a21c-97812ccfbaee`; `/feedback` ID pending | GPT-5.6 Sol owned integration and repair; GPT-5.6 Luna xhigh independently checked app-server schema, migration, routing, and safety edges. Grok 4.5 supplied the bounded metadata-model draft, which the parent audited and repaired. | Persist metadata but never bytes; require current protocol and per-model image evidence; send only bounded local-image references; fail closed for missing, oversized, stale, or unsupported images; keep ordinary-file path compatibility without claiming image delivery. | 932 deterministic fallback checks passed; 51 native test files / 528 declarations inventoried but native Swift Testing was unavailable; arm64 app packaging and bundle verification passed. The built app launched without crash, but Computer Use accessibility capture timed out, so interaction QA remains a stated limitation. |
| 2026-07-15 | Worktree-safe Code checkpoints, Review notes, and guarded revert | `codex/code-mode-checkpoints` (implementation commit recorded in final handoff) | Current Codex desktop task; `/feedback` ID pending | GPT-5.6 Codex implemented the Git-native checkpoint service, then used a bounded Grok 4.5 implementation/review pass and independently audited and repaired staged-file capture, symlink privacy, mode/divergence checks, CAS refs, review-body validation, in-flight refresh clobbering, and cancellation-time before-checkpoint recovery. A requested Luna xHigh audit was attempted but unavailable because the subagent account hit its usage limit. | Checkpoints are scoped by canonical worktree/common-Git identity plus session/run ownership; tracked snapshots use alternate-index trees and namespaced refs; untracked bytes never enter the store; Review notes are path/line-bound; revert is previewed, token-bound, conflict-aware, and explicitly confirmed without reset/checkout/restore shortcuts. | `./script/build_and_run.sh --test` passed 970 deterministic fallback checks (native Swift Testing unavailable); `./script/build_and_run.sh --verify` built and bundle-verified the arm64 app; Computer Use QA opened Code → Inspector → Review and verified the accessible empty-state surface. |
| 2026-07-15 | Screenshot capture input and computer-frame foundation | This dated commit | Codex task delegated from `019f6235-6ecb-76f0-a21c-97812ccfbaee`; `/feedback` ID pending | GPT-5.6 Sol/Luna authored capture foundation; residual integration completed after rate-limit interruption. | User-initiated region/window capture, Screen Recording vs Accessibility separation, durable capture storage, attachment transport fail-closed, and bounded computer-frame accumulation without mouse/keyboard mediation. | Inventory and fallback verification recorded on integration branch. |
