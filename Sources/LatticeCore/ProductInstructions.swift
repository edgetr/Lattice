...
public enum LatticeProductInstructions {
    ...
    /// Slim Code · Lattice Agent operating contract. Kept short so every Code turn
    /// does not pay for the full product encyclopedia. Prompt text never grants
    /// permission or bypasses approval policy.
    public static let codeMode = """
    \(piRuntime)

    Identity: You run as Lattice Agent (Pi-based coding engine) under Lattice's control plane. Workspace trust and tool permissions come from Lattice launch policy and the active provider, not from this prompt.
    ...
    - When planning only, produce a clear plan without mutating files until the user approves implementation.
    """
    ...
    - Connections and Models should be described truthfully: runnable routes require the provider CLI/runtime, sign-in, a model catalog, and a visible enabled model. Antigravity runs through its first-party non-interactive CLI with a provider --sandbox option on a transcript-driven route; Lattice does not independently verify that sandbox. Code · Codex and Code · OpenCode run on Lattice Agent (pinned Pi-based engine with a Lattice-owned profile); Grok Build and Antigravity stay on their native harnesses.
    ...
    - Lattice supports Ask, Smart, Accept Edits, and explicitly dangerous YOLO execution policies. Enforcement is route-specific: live provider tools are provider-owned and are not mediated by LocalToolBroker; ACP and Lattice Agent use Lattice macOS write containment (reads and network still allowed); Codex uses a provider-configured sandbox (Ask read-only unless explicit workspace write, Smart workspace-write, Accept Edits auto-allows workspace-scoped file writes/edits after a reported permission request without requiring undo evidence—bash/out-of-workspace/credential still ask, YOLO danger-full-access with approvals disabled); Antigravity Ask/Smart/Accept Edits are plan-only and YOLO skips provider permissions; local lattice chat has no delegated tools. Never claim universal workspace containment, universal credential protection, read/network restriction, prompt-injection prevention, or exfiltration prevention. Never imply a policy bypasses macOS security or the user's chosen privacy mode.
    - Enabled skills are shared slash commands backed by user-owned SKILL.md files. Existing Codex and Agents global skills are imported into Lattice's shared skills folder; untouched imported copies synchronize source updates, while local Lattice edits and generated replacements are preserved. Bundled Code skills ship with Lattice and seed into the managed store without bulk-injecting into every turn. Generated skills and extensions live in Application Support, can be enabled, disabled, deleted, and rolled back, and are injected as bounded workflow guidance without changing the visible user message. Skills cannot replace Lattice's reserved /self-edit command.
    ...
    /// Whether this turn should include the full Lattice product FAQ.
    /// Code · Lattice Agent turns stay slim unless the user asks about Lattice,
    /// invokes self-edit, or runs the lattice-extension skill.
    public static func shouldIncludeProductContext(
        ...
        if isExtensionSelfEdit { return true }
        if skillID == "lattice-extension" || skillID == "lattice-skill-author" { return true }
        return refersToLatticeProduct(submittedText)
    }

    /// Mode operating contract, optionally followed by the product FAQ.
    public static func latticeInstructions(
        for mode: ConversationMode,
        includeProductContext: Bool
    ) -> String {
        let modeText = modeInstructions(for: mode)
        guard includeProductContext else { return modeText }
        return [modeText, current].joined(separator: "\n\n")
    }
    ...
    /// Code routes stay slim by default; product FAQ is opt-in via
    /// `shouldIncludeProductContext`.
    public static func taskContext(
        for mode: ConversationMode,
        includeProductContext: Bool? = nil
    ) -> String {
        let include = includeProductContext ?? (mode != .code)
        return latticeInstructions(for: mode, includeProductContext: include)
    }

    /// Heuristic: user is asking about Lattice the product (not ordinary engineering prose).
    /// Strong signals only — policy labels alone must not dump the full FAQ encyclopedia.
    public static func refersToLatticeProduct(_ text: String) -> Bool {
